...
export default function friendsPlugin(schema, options) {
    options = { ...defaultOptions, ...options };

    schema.add({
        [options.fieldName]: { type: [relationSchema] }
    });
    ...
    function getRelation(user) {
        const relations = getRelations.call(this);
    ...
    async function addRelation(user, status) {
        const userDoc = await fetchUser.call(this, user);
        if (!userDoc) {
            return null;
        }
        let relation = await getRelation.call(this, userDoc);
        if (!relation) {
        ...
        return await getRelation.call(this, userDoc);
    }
    ...
    schema.methods.requestFriend = async function requestFriend(user) {
        ...
        let relation = this.getRelation.call(this, userDoc);
        if (relation === null) {
