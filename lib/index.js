...
module.exports = function friendsPlugin(schema, options) {
    ...
    async function removeRelation(user) {
        ...
        if (relation) {
            selfDoc[options.fieldName].pull(relation);
            await selfDoc.save();
        }
        const otherRelation = await userDoc.getRelation(selfDoc);
        if (otherRelation) {
            userDoc[options.fieldName].pull(otherRelation);
            await userDoc.save();
        }
    }
    ...
    async function updateRelation(user, status) {
        const selfDoc = await this.fetchUser(this);
        const relation = selfDoc.relations.find((relation) => user.equals(relation.user));
        if (relation !== -1) {
            selfDoc[options.fieldName].pull(relation);
            relation.status = status;
            selfDoc[options.fieldName].push(relation);
            await selfDoc.save();
            let newDoc = await selfDoc.constructor.findById(selfDoc.id);
        }
        return relation;
    }
